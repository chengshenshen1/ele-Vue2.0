			...
			<div id="J_lyricScrollView" class="ks-scroll-view">
				<div class="ks-scroll-view-content">
					<div class="ui-player-lrc-wrap" id="J_playerLrc">
						<ul>
							<li class="ui-lrc-line ui-lrc-current">{{_lrc[0][1]}}</li>
							<li class="ui-lrc-line" v-for="lrc in _lrc">{{lrc[1]}}</li>
						</ul>
					</div>
				</div>
			</div>
...
export default {
	props: {
		playing: {
			...
			default() {
				return {
				'name': '演员',
				'src': 'http://og06z4hqx.bkt.clouddn.com/%E8%96%9B%E4%B9%8B%E8%B0%A6%20-%20%E6%BC%94%E5%91%98.mp3',
				'singer': '薛之谦',
				'album': '绅士',
				'avatar': 'http://img.xiami.net/images/album/img44/7244/14320912641433747785_2.jpg',
				'islike': false,
				'lrc': `
					[00:00.32]演员
					[00:01.00]
	...
	computed: {
		_lrc() {
			let item = this.playing;
			if (item.lrc) {
				let lrcArr = item.lrc.split('[');
				let lrcFormat = [];

				lrcArr.forEach(function(str) {
					let temp = str.split(']');
					temp[0] = +temp[0].split(':')[0] * 60 + (+temp[0].split(':')[1]);
					if (temp[1] && temp[1] !== ' ') {
						lrcFormat.push(temp);
					}
				});
				item.lrc = lrcFormat.slice(1);
			} else {
				item.lrc = undefined;
			}
			return item.lrc;
...
<style lang="less" rel="stylesheet/less">@import "./mainlrc.less";</style>
